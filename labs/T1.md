...
et l'image disque sur <http://tahiti.prism.uvsq.fr/2013-12-20-wheezy-raspbian.img.zip>.

Une fois le noyau et l'image téléchargés, vous pouvez lancer votre émulateur ARM avec les commandes suivante:

~~~bash
$ unzip 2013-12-20-wheezy-raspbian.img.zip # à ne faire que la première fois

$ qemu-system-arm -M versatilepb -cpu arm1176 -m 256 \
  -redir tcp:2222::22 -hda 2013-12-20-wheezy-raspbian.img \
  -kernel kernel-qemu -append "root=/dev/sda2"
~~~
...
Puis on peut allumer la LED en écrivant la valeur ``1<<16`` à l'adresse ``gpio_map+40``.

On peut éteindre la LED en écrivant la valeur ``1<<16`` à l'adresse ``gpio_map+28``.
